...
class CharLoader():
    ...
    def __init__(self, batch_size, 
                 ...
                 fonts_dir_path="characters/font/",
...
# =============================================================================
    ...
    def load_data(self, type="train", onehot=True):
        if type=="test":
            X = self.X_test
            y = self.y_test
        elif type=="valid":
...
def preprocess_emnist_data(X, y, subset_size):
    X = X.reshape( (X.shape[0], 28, 28), order='F')
    X = np.expand_dims(X, axis=-1) 
    X = tf.image.resize(X, size=(32,32), preserve_aspect_ratio=True)
    X = np.squeeze(X)
    X = X[:subset_size]
    y = np.squeeze(y[:subset_size])
    return X, y
...
def augment_img(images, output, augmenter):
    ...
    image = tf.numpy_function(augmenter.augment_image, [images], tf.uint8)
    return image, output
...
if __name__=="__main__":
    data = CharLoader(batch_size=16)
    train, steps, num_classes = data.load_data(onehot=False)
    for img, label in train.take(20):
        show_img(img[0])
        print(label[0])
